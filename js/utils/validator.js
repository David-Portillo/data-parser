...
const adjutant = {
  advisable: (r, v, field) => {
    ...
    if (value.length === 0)
      return {
        passed: false,
        message: fieldMessage(`advisable->${field}`, null, r, field),
        advisable: true,
      };
  ...
  required: (r, v, field) => {
    ...
    if (rule && value.length === 0)
      return {
        passed: false,
        message: fieldMessage("required", null, rule, field),
      };
    return { passed: true, message: "" };
  },
  minLength: (r, v, field) => {
    ...
    if (value.length < rule)
      return {
        passed: false,
        message: fieldMessage("minLength", value, rule, field),
      };
    return { passed: true, message: "" };
  },
  maxLength: (r, v, field) => {
    ...
    if (value.length > rule)
      return {
        passed: false,
        message: fieldMessage("maxLength", value, rule, field),
      };
    return { passed: true, message: "" };
  },
  numbersOnly: (r, v, field) => {
    const value = parseValue(v);
    const { minValue = null, maxValue = null } = parseRule({
      rule: r || "{}",
      isObject: true,
    });
    ...
    if (isNaN(value))
      return {
        passed: false,
        message: fieldMessage("numbersOnly", value, r, field),
      };
  ...
  dateField: (r, v, field) => {
    const value = parseValue(v);
    const { format = moment.HTML5_FMT.DATE } = parseRule({
      rule: r || "{}",
      isObject: true,
    });
    const separator =
      format.split("").find((s) => s === "." || s === "/" || s === "-") || "-";
    ...
    if (value.length > 0) {
      ...
      if (hasInvalidChar)
        return {
          passed: false,
          message: fieldMessage("dateField->format", value, format, field),
        };
      ...
      if (!moment(value, format, true).isValid()) {
        ...
        if (invalidAt === 0)
          return {
            passed: false,
            message: fieldMessage("dateField->year", value, format, field),
          };
        if (invalidAt === 1)
          return {
            passed: false,
            message: fieldMessage("dateField->month", value, format, field),
          };
        if (invalidAt === 2)
          return {
            passed: false,
            message: fieldMessage("dateField->day", value, format, field),
          };

        return {
          passed: false,
          message: fieldMessage("dateField->unknown", value, format, field),
        };
      }
    }
